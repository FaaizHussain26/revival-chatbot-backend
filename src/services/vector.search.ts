    ...
    const results = await Chunk.aggregate([
      {
        $vectorSearch: {
          ...
          limit: limit,
          filter: {
            "metadata.category": { $in: getCategoriesToSearch(category) },
          },
        },
      },
      {
...
function getCategoriesToSearch(selected: KnowledgeCategory | null) {
   if (!selected) {
    return [KnowledgeCategory.Scraped];
  }
  return [selected, KnowledgeCategory.Scraped];
}
